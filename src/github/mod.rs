...
impl GitHubClient {
    ...
    pub async fn list_environments(&self, owner: &str, repo: &str) -> Result<Vec<String>> {
        ...
        let response: EnvironmentsResponse = response
            .json()
            .await
    ...
    pub async fn list_secrets(&self, owner: &str, repo: &str, environment: &str) -> Result<Vec<Secret>> {
        let url = format!(
        ...
        let response: SecretsResponse = response
            .json()
            .await
            .context("Failed to parse response")?;
    ...
    pub async fn get_secret(&self, owner: &str, repo: &str, environment: &str, secret_name: &str) -> Result<Secret> {
        let url = format!(
            "https://api.github.com/repos/{}/{}/environments/{}/secrets/{}",
    ...
    async fn get_repository_public_key(&self, owner: &str, repo: &str) -> Result<RepositoryPublicKey> {
        let url = format!(
        ...
        let public_key: RepositoryPublicKey = response
            .json()
            .await
            .context("Failed to parse response")?;
